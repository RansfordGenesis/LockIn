...
import { PlanSummary, MAX_PLANS_PER_USER, PlanCategory } from "@/types/multiplan";
...
interface AppState {
  ...
  // Save session credentials (for tab refresh)
  saveSession: (email: string, phone: string) => void;
...
export const useAppStore = create<AppState>()((set, get) => ({
  ...
  removePlanSummary: (planId) =>
    set((state) => {
      ...
      if (state.activePlanId === planId) {
        newActivePlanId = newPlans.length > 0 ? newPlans[0].planId : null;
      }
  ...
  // Save session credentials for tab refresh
  saveSession: (email, phone) => {
    if (globalThis.window !== undefined) {
      globalThis.sessionStorage.setItem('lockin-email', email);
      globalThis.sessionStorage.setItem('lockin-phone', phone);
    }
  },

  // Initialize app - try to restore session from sessionStorage
  initializeApp: async () => {
    const state = get();
    ...
    try {
      // Check if we have a session stored (email in sessionStorage for current tab)
      let sessionEmail: string | null = null;
      let sessionPhone: string | null = null;
      
      if (globalThis.window !== undefined) {
        sessionEmail = globalThis.sessionStorage.getItem('lockin-email');
        sessionPhone = globalThis.sessionStorage.getItem('lockin-phone');
      }
      ...
      if (sessionEmail && sessionPhone) {
        ...
        const response = await fetch("/api/auth/login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: sessionEmail, phone: sessionPhone }),
        });
        ...
        if (data.success && data.plan) {
          ...
          set({
            user: {
              ...
              settings: {
                scheduleType: data.plan.scheduleType || "weekdays",
                timeCommitment: data.plan.timeCommitment || "1hr-daily",
                reminderTime: "09:00",
                timezone: "Africa/Accra",
                emailNotifications: true,
                smsNotifications: true,
              },
              currentStreak: data.userState?.currentStreak || 0,
              longestStreak: data.userState?.longestStreak || 0,
              totalPoints: data.userState?.totalPoints || 0,
              completedDays: 0,
            },
            plans: data.plans || [],
            activePlanId: data.activePlanId,
            plan: data.plan,
            completedTasks: data.userState?.completedTasks || {},
            totalPoints: data.userState?.totalPoints || 0,
            currentStreak: data.userState?.currentStreak || 0,
            longestStreak: data.userState?.longestStreak || 0,
            lastCheckIn: data.userState?.lastCheckIn || null,
            dailyCheckIns: data.userState?.dailyCheckIns || {},
            globalTotalPoints: data.globalStats?.totalPoints || 0,
            globalCurrentStreak: data.globalStats?.currentStreak || 0,
            globalLongestStreak: data.globalStats?.longestStreak || 0,
            currentView: "daily",
            currentDate: format(new Date(), "yyyy-MM-dd"),
            isInitialized: true,
            isLoading: false,
          ...
          return;
        } else if (data.success && data.needsPlan) {
          // User exists but has no plan
          set({
            user: {
              userId: data.user.email,
              email: data.user.email,
  ...
  clearAll: () => {
    ...
    if (globalThis.window !== undefined) {
      globalThis.sessionStorage.removeItem('lockin-email');
      globalThis.sessionStorage.removeItem('lockin-phone');
    }
